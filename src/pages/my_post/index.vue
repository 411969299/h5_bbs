      ...
      <li v-for="item in myPostLists">
        ...
        <div class="con">
          <div class="clearfix" v-link="{path: '/forum_detail?postId=' + item.postId}">
            ...
            <div class="post_info">
              <h6>{{ item.fullname }}</h6>
              <p v-if="item.postType === 2">
                <i class="lable">打卡</i>
                &nbsp;{{ item.postTitle }}
                <span>({{ item.finishDays }}/{{ item.goalDays }})</span></p>
...
<script>
  ...
  import { getMyPostList } from '../../vuex/post/actions'
    ...
    watch: {
      lists () {
        ...
        if (this.lists.length !== 0) {
          this.page += 1
          this.loading = false
          this.myPostLists = this.myPostLists.concat(this.lists)
        } else {
          this.allLoaded = true
        }
      },
